...
function normalizeWorkOrderDraft(value, runId) {
    const id = typeof value.id === 'string' ? value.id.trim() : '';
    const title = typeof value.title === 'string' ? value.title.trim() : '';
    const objective = typeof value.objective === 'string' ? value.objective.trim() : '';
    if (!id || !title || !objective)
        throw new DungeonError('INVALID_ARGS', 'workOrder needs id, title, and objective');
        ...
        ctx.tools.register(defineTool({
            ...
            async execute(args, exec) {
                ...
                if (args.action === 'create') {
                    ...
                    return json(service.createTask(caller, args.runId, normalizeWorkOrderDraft(args.workOrder, args.runId)));
                }
