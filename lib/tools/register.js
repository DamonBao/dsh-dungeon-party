...
async function runVerification(command, timeoutMs, cwd) {
    ...
    const disposers = [
            ...
            async execute(args, exec) {
                ...
                if (args.action === 'create') {
                    ...
                    while (run.tasks[`task-${ordinal}`])
                        ordinal += 1;
                    return json(service.createTask(caller, args.runId, normalizeWorkOrderDraft(args.workOrder, args.runId, `task-${ordinal}`)));
                }
        ...
        ctx.tools.register(defineTool({
            ...
            async execute(args, exec) {
                ...
                if (result.timedOut)
                    return json({ code: 'VERIFICATION_TIMEOUT', command: args.command, durationMs: result.durationMs, outputExcerpt: result.outputExcerpt });
        ...
        ctx.tools.register(defineTool({
            ...
            async execute(args, exec) {
                ...
                const run = service.finishRun(caller, args.runId, args.resultSummary, fingerprint, () => computeWorkspaceFingerprint(current.workspaceRoot, service.getFingerprintIgnoreScopes()));
                await agentManager?.disposeRun(args.runId);
                return json(summarizeRun(run));
            },
        })),
