 ...
 */
export function createReadableRunId(at = new Date(), suffix = defaultRunIdSuffix()) {
...
export class DungeonService {
    ...
    preflightTaskAssignment(actor, runId, taskId, slot) {
        ...
        const busySlotTask = Object.values(run.tasks).find((candidate) => candidate.workOrder.id !== taskId &&
     ...
     */
    serialWriteBlocked(runId, taskId) {
    ...
    findSerialWriteBlocker(run, task) {
        if (run.scopeEnforcementMode !== 'serial')
            return undefined;
        if (task.workOrder.writeScopes.length === 0)
            return undefined;
        const leasedBlocker = Object.values(run.tasks).find((candidate) => candidate.workOrder.id !== task.workOrder.id &&
            candidate.workOrder.writeScopes.length > 0 &&
            candidate.activeLease !== undefined);
        if (leasedBlocker)
            return leasedBlocker;
        ...
        const awaiting = Object.values(run.tasks).filter((candidate) => candidate.workOrder.writeScopes.length > 0 &&
            candidate.status === 'ready' &&
            candidate.ownerSlot !== undefined);
        const head = awaiting[0];
        if (!head || head.workOrder.id === task.workOrder.id)
            return undefined;
        return head;
    }
    /**
     ...
     */
    assertSerialWriteAssignable(run, task) {
        const blocker = this.findSerialWriteBlocker(run, task);
        assert(!blocker, 'WRITE_DISPATCH_SERIALIZED', `Strict scope mode serializes write leases; ${blocker?.workOrder.id ?? 'another write task'} is ahead in the queue. Wait with party_wait until it completes before assigning or claiming another write task.`);
    }
    ...
    claimTask(actor, runId, taskId) {
        const run = this.requireRun(runId);
        assert(run.phase === 'EXECUTING' || run.phase === 'REPAIR', 'INVALID_PHASE', 'Tasks can only be claimed during execution or repair');
        ...
        assert(!activeDps.has(actorSlot), 'LEASE_ALREADY_HELD', `Slot ${actorSlot} already holds an active lease; submit or expire it before claiming another task`);
        assert(activeDps.size < this.config.maxConcurrentDps, 'MAX_CONCURRENT_DPS', `At most ${this.config.maxConcurrentDps} DPS slots may hold leases concurrently`);
    ...
    submitExecution(actor, runId, report) {
        ...
        assert(submitNowMs <= Date.parse(task.activeLease.expiresAt) ||
            (protectedUntilMs !== undefined && submitNowMs <= protectedUntilMs), 'LEASE_EXPIRED', 'Task lease has expired');
    ...
    recordVerificationCommand(actor, runId, result) {
        const run = this.requireRun(runId);
    ...
    sweepExpiredState(runId) {
        ...
        for (const task of Object.values(run.tasks)) {
            if (task.activeLease && now > Date.parse(task.activeLease.expiresAt)) {
                ...
                if (this.submitProtectedUntil(runId, task.workOrder.id, now))
                    continue;
    ...
    resumeDispatch(actor, runId) {
        const run = this.requireTank(actor, runId);
        assert(run.controlState === 'recovering' || run.controlState === 'throttled' || run.controlState === 'paused', 'DISPATCH_NOT_PAUSED', 'Dispatch is not ready to resume');
        ...
        if (run.controlState === 'paused') {
            ...
            assert(run.slots.healer.readiness === 'healthy' && run.slots.tank.lifeState === 'alive', 'HEALER_NOT_RECOVERED', 'A paused run can only resume after the healer readiness is healthy and the tank is alive');
        }
    ...
    observeHealthSignal(runId, input) {
        ...
        const recent = updated.healthSignals.filter((item) => item.slot === input.slot && Date.parse(item.observedAt) >= cutoff);
    ...
    reassignTask(actor, runId, taskId, ownerSlot) {
        ...
        assert(task.interruptState === 'completed' || ownerDown, 'INTERRUPT_NOT_CONFIRMED', 'Original Turn termination is not confirmed and the current owner is not down');
    ...
    observeCommanderLoad(runId, observation) {
        ...
        if (commanderLoad === 'overloaded' && run.controlState !== 'throttled') {
            ...
            const checkpoint = {
                ...
                memberReadiness: Object.fromEntries(slots.flatMap((slot) => current.slots[slot].readiness ? [[slot, current.slots[slot].readiness]] : [])),
     ...
     */
    retryExecution(actor, runId, taskId, reason) {
        const run = this.requireTank(actor, runId);
        this.assertMutable(run);
        assert(run.phase === 'EXECUTING' || run.phase === 'REPAIR', 'INVALID_PHASE', 'Execution retries require EXECUTING or REPAIR');
        const task = this.requireTask(run, taskId);
        assert(task.status === 'blocked' || task.status === 'failed', 'TASK_NOT_RETRYABLE', `Only blocked or failed tasks can be retried (current status: ${task.status})`);
        ...
        assert(task.executionRetries < this.config.maxExecutionRetries, 'RETRY_LIMIT_EXCEEDED', `Task ${taskId} exceeded the execution retry limit (${this.config.maxExecutionRetries})`);
    ...
    async finishRun(actor, runId, resultSummary, workspaceFingerprint, recomputeFingerprint) {
        ...
        const chained = previous.catch(() => undefined).then(() => this.finishRunUnlocked(actor, runId, resultSummary, workspaceFingerprint, recomputeFingerprint));
        this.completionLocks.set(runId, chained);
        try {
            return await chained;
        }
        finally {
            if (this.completionLocks.get(runId) === chained)
                this.completionLocks.delete(runId);
        }
    }
    async finishRunUnlocked(actor, runId, resultSummary, workspaceFingerprint, recomputeFingerprint) {
        const run = this.requireTank(actor, runId);
        ...
        if (recomputeFingerprint) {
            ...
            if (stateChanged || actualFingerprint !== workspaceFingerprint) {
                ...
                if (stateChanged) {
                    throw new DungeonError('COMPLETION_CONFLICT', 'Run state changed during completion; re-check party state and validation before finishing');
                }
                throw new DungeonError('WORKSPACE_CHANGED_DURING_COMPLETION', `Workspace changed during completion: expected fingerprint ${workspaceFingerprint} but recomputed ${actualFingerprint}`);
            }
        }
    ...
    protectSubmit(runId, taskId) {
        ...
        const perRun = this.submitProtections.get(runId) ?? new Map();
    ...
    getExecutionGuardView(runId, slot) {
        const run = this.requireRun(runId);
    ...
    reduce(current, event) {
        ...
        switch (event.type) {
            ...
            case 'dungeon/verification-command-run':
                run.verificationRuns.push(payload);
                break;
            case 'dungeon/task-submitted': {
                ...
                run.slots[report.slot].activityState = 'idle';
                break;
            }
            ...
            case 'dungeon/task-lease-revoked': {
                ...
                if (payload.reason === 'lease-expired')
                ...
                if (task.ownerSlot) {
                    ...
                    if (ownerBinding.lifeState === 'alive')
                        ownerBinding.activityState = 'idle';
                }
                break;
            }
