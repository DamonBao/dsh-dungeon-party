...
function readPackedRef(gitDir, refName) {
    try {
        for (const line of readFileSync(join(gitDir, 'packed-refs'), 'utf8').split('\n')) {
            ...
            if (commit !== undefined && name === refName && isCommitId(commit))
                return commit;
        }
    }
    catch {
...
function resolveHeadCommit(gitDir) {
 ...
 */
function resolveGitHeadCommit(workspaceRoot) {
    try {
...
export function computeWorkspaceFingerprintFromSnapshot(workspaceRoot, snapshot) {
...
export function computeWorkspaceFingerprint(workspaceRoot, ignoreScopes = []) {
    return computeWorkspaceFingerprintFromSnapshot(workspaceRoot, createWorkspaceSnapshot(workspaceRoot, ignoreScopes));
}
