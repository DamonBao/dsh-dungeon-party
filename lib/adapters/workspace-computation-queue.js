...
import { computeWorkspaceFingerprintFromSnapshot, } from './workspace-fingerprint.js';
...
export class WorkspaceComputationQueue {
    ...
    queued = [];
    active;
    snapshot(workspaceRoot, ignoreScopes) {
    ...
    async fingerprint(workspaceRoot, ignoreScopes) {
    ...
    ensureWorker() {
        ...
        worker.on('message', (reply) => {
            ...
            if (reply.error !== undefined)
                ...
                job.resolve(reply.snapshot);
            else
                job.reject(new Error('Workspace computation worker returned no snapshot'));
        ...
        worker.on('exit', (code) => {
            if (code !== 0 && this.worker === worker)
                fail(new Error(`Workspace computation worker exited with code ${code}`));
        });
        this.worker = worker;
        return worker;
    }
