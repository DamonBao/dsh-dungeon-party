...
import { diffWorkspaceSnapshots, } from './workspace-fingerprint.js';
...
export class PartyAgentManager {
    ...
    constructor(service, agents, presets, childRoute = {}, clockMs = Date.now) {
    ...
    observeSessionActivity(sessionId, atMs = this.clockMs()) {
     ...
     */
    childAgentOptions(tankAgent) {
        return { ...tankAgent.options, ...this.childRoute };
    }
    async restoreBoundParty(actor, runId) {
        const run = this.service.getRunForActor(actor, runId);
        for (const slot of ['dps-1', 'dps-2', 'dps-3', 'healer']) {
            if (run.slots[slot].currentSessionId)
    ...
    async runWatchdog() {
        for (const runId of this.service.listActiveRunIds()) {
            ...
            if (!tankSessionId)
                continue;
            const tankActor = { sessionId: tankSessionId };
            ...
            // stay fully audited.
            const missingAudit = Object.values(swept.tasks).some((task) => task.activeLease && task.ownerSlot && !this.leaseAudits.has(`${runId}:${task.workOrder.id}`));
            if (missingAudit)
                await this.rebuildMissingLeaseAudits(runId).catch(() => undefined);
            ...
            for (const task of Object.values(swept.tasks)) {
                if (task.status !== 'ready' || !task.ownerSlot || task.activeLease)
                    continue;
                const ownerBinding = swept.slots[task.ownerSlot];
                if (!ownerBinding.currentSessionId || ownerBinding.lifeState !== 'alive')
                    continue;
                const redispatchKey = `${runId}:${task.workOrder.id}`;
                const nowMs = this.clockMs();
                if (nowMs < (this.redispatchAt.get(redispatchKey) ?? 0))
                    continue;
            ...
            for (const task of Object.values(swept.tasks)) {
                if (task.status !== 'running' || !task.activeLease || !task.ownerSlot)
                    continue;
                ...
                if (!after || after.progressState === before)
                    continue;
                if (after.progressState === 'suspected-stalled') {
                    ...
                    this.agents.get(tankSessionId)?.send(createUserMessage({
                        content: [{
                                type: 'text',
    ...
    nudgeAfterTurnEnd(sessionId) {
        ...
        for (const runId of this.service.listActiveRunIds()) {
            ...
            const slot = run.phase === 'EXECUTING' || run.phase === 'REPAIR'
                ? ['dps-1', 'dps-2', 'dps-3'].find((candidate) => run.slots[candidate].currentSessionId === sessionId)
                : undefined;
            if (!slot)
                continue;
     ...
     */
    serialWriteBlocked(runId, task) {
        return this.service.serialWriteBlocked(runId, task.workOrder.id);
    }
    async withDispatchLock(runId, operation) {
        ...
        try {
            ...
            if (this.dispatchLocks.get(runId) === chained)
                this.dispatchLocks.delete(runId);
        }
    }
    async withWorkspaceAuditLock(runId, operation) {
        ...
        try {
            ...
            if (this.workspaceAuditLocks.get(runId) === chained)
                this.workspaceAuditLocks.delete(runId);
        }
    }
    async dispatchAvailableTasksUnlocked(actor, runId) {
        const assigned = [];
        const run = this.service.getRunForActor(actor, runId);
        if (run.phase !== 'EXECUTING' && run.phase !== 'REPAIR')
            return assigned;
        ...
        const freeSlots = ['dps-1', 'dps-2', 'dps-3'].filter((slot) => {
        ...
        for (const task of readyTasks) {
            ...
            if (this.serialWriteBlocked(runId, task))
                continue;
     ...
     */
    notifyDrainedExecution(runId) {
        try {
            const run = this.service.getRun(runId);
            if (run.phase !== 'EXECUTING' && run.phase !== 'REPAIR')
                return;
            if (this.drainNoticeTaskSetVersions.get(runId) === run.taskSetVersion)
                return;
            ...
            const hasOpenWork = tasks.some((task) => task.status === 'pending' || task.status === 'ready' || task.status === 'running' || task.activeLease);
            if (hasOpenWork)
                return;
            // Optional tasks reported blocked/failed do not gate VALIDATING, so
            // only incomplete required tasks hold the notice back.
            if (tasks.some((task) => task.workOrder.required && task.status !== 'completed'))
                return;
            const recoveryInFlight = run.resurrectionRequests.some((request) => request.status === 'issued' || request.status === 'consumed') ||
    ...
    async recoverCommander(actor, runId, ticketId) {
        ...
        if (!completed)
            throw new DungeonError('COMMANDER_RECOVERY_INCOMPLETE', 'Commander recovery produced no ticket result');
    ...
    async claimTaskWithAudit(actor, runId, taskId) {
    ...
    async beginLeaseAudit(actor, runId, taskId, leaseId) {
        await this.withWorkspaceAuditLock(runId, () => this.captureLeaseAudit(actor, runId, taskId, leaseId));
    }
    async submitExecutionWithAudit(actor, runId, report) {
    ...
    async auditWorkspaceBeforeSubmit(actor, runId, report) {
        await this.withWorkspaceAuditLock(runId, () => this.auditWorkspace(actor, runId, report));
    }
    async captureLeaseAudit(actor, runId, taskId, leaseId) {
        const run = this.service.getRunForActor(actor, runId);
     ...
     */
    async rebuildMissingLeaseAudits(runId) {
        await this.withWorkspaceAuditLock(runId, async () => {
            const run = this.service.getRun(runId);
            if (run.phase !== 'EXECUTING' && run.phase !== 'REPAIR')
                return;
            for (const task of Object.values(run.tasks)) {
                if (!task.activeLease || !task.ownerSlot)
                    continue;
                const key = `${runId}:${task.workOrder.id}`;
                if (this.leaseAudits.has(key))
                    continue;
    ...
    async auditWorkspace(actor, runId, report) {
        const run = this.service.getRunForActor(actor, runId);
        ...
        const isByproduct = (path) => byproductScopes.some((scope) => path === scope || matchesGlob(path, scope));
        ...
        if (run.scopeEnforcementMode === 'serial') {
            ...
            const actual = changedFiles.filter((path) => run.tasks[report.taskId].workOrder.writeScopes.some((scope) => path === scope || matchesGlob(path, scope))).sort();
            ...
            if (unreported.length > 0 || overReported.length > 0) {
                throw new DungeonError('CHANGED_FILES_MISMATCH', `Reported changedFiles do not match the host-observed serial workspace delta. Unreported changes: [${boundedPathList(unreported)}]; reported but not observed: [${boundedPathList(overReported)}]. Toolchain byproducts are excused automatically; correct the list and resubmit.`);
            }
        }
    }
     ...
     */
    async refreshLeaseAudit(runId, taskId) {
        await this.withWorkspaceAuditLock(runId, async () => {
            ...
            const run = this.service.getRun(runId);
            const task = run.tasks[taskId];
            if (!task?.activeLease || task.activeLease.leaseId !== audit.leaseId)
                return;
            const next = await workspaceComputationQueue.snapshot(run.workspaceRoot, this.service.getFingerprintIgnoreScopes());
            for (const path of diffWorkspaceSnapshots(audit.snapshot, next))
                audit.accumulated.add(path);
            audit.snapshot = next;
        });
    }
    async interruptTask(actor, runId, taskId, turnId) {
        ...
        const quarantinedFiles = audit
            ? diffWorkspaceSnapshots(audit.snapshot, await workspaceComputationQueue.snapshot(run.workspaceRoot, this.service.getFingerprintIgnoreScopes()))
            : [];
        this.leaseAudits.delete(`${runId}:${taskId}`);
     ...
     */
    ensureGuardInstalled(agentCtx, runId, slot) {
        if (slot === 'healer')
            return;
        this.guardBindings.set(agentCtx, { runId, slot: slot });
        if (this.guardedContexts.has(agentCtx))
            return;
        this.guardedContexts.add(agentCtx);
        this.installExecutionGuard(agentCtx);
    }
    installExecutionGuard(agentCtx) {
    ...
    ensureSubagentDescriptor(agent, runId, slot) {
        if (agent.session.events.some((event) => event.type === 'subagent/descriptor'))
            return;
    ...
    async restoreMember(actor, runId, slot, sessionId) {
        ...
        if (live) {
            ...
            if (liveCtx)
                this.ensureGuardInstalled(liveCtx, runId, slot);
...
function boundedPathList(paths, limit = 20) {
 ...
 */
function findSymlinkEscape(root, absolutePath) {
    try {
        ...
        if (!contained(realpathSync(anchor)))
            return `${anchor} -> ${realpathSync(anchor)}`;
        if (existsSync(absolutePath)) {
            const targetReal = realpathSync(absolutePath);
            if (!contained(targetReal))
                return `${absolutePath} -> ${targetReal}`;
        }
        else {
            try {
